...
async def serve_webapp():
    ...
    <html>
        <body style="background:#0f0f1a; color:white; font-family:Arial; padding:40px; text-align:center;">
            ...
            <p>Looking for: {WEBAPP_DIR}/index.html</p>
            <p>Current BASE_DIR: {BASE_DIR}</p>
        </body>
    </html>
    """)
...
@app.get("/health")
async def health():
    return {"status": "ok", "webapp_dir": WEBAPP_DIR, "exists": os.path.exists(WEBAPP_DIR)}
...
async def get_user_applications(
    ...
    return [
        ApplicationResponse(
            id=row[0], bank_name=row[1], amount_uah=row[2], payment_code=row[3],
            status=row[4], status_label=STATUS_LABELS.get(row[4], row[4]), created_at=row[5]
        ) for row in rows
    ]

@app.get("/api/applications/count")
async def get_applications_count(user: dict = Depends(get_current_user), status: Optional[str] = Query(None)):
    tg_id = user.get("id")
...
async def create_application(data: ApplicationCreate):
    ...
    if has_requisites:
        ...
        await db.create_notification(tg_id, "requisites", "Реквизиты получены", 
                                     f"Заявка #{app_id}: реквизиты для оплаты",
                                     json.dumps({"app_id": app_id}))
        return CreateAppResponse(
            success=True, app_id=app_id, message="Заявка создана! Реквизиты получены автоматически.",
            requisites=requisites, expires_at=(dt.datetime.utcnow() + dt.timedelta(minutes=20)).isoformat() + "Z",
            bank_name=bank["bank_name"], country_name=country_name, amount=data.amount_uah
        )
    else:
        return CreateAppResponse(
            success=True, app_id=app_id, message="Заявка создана! Ожидайте выдачи реквизитов оператором.",
            requisites=None, expires_at=None, bank_name=bank["bank_name"], country_name=country_name, amount=data.amount_uah
        )
...
async def get_application_detail(app_id: int, user: dict = Depends(get_current_user)):
    ...
    STATUS_LABELS = {
        "WAITING_MERCHANT": "Ожидает мерчанта", "MERCHANT_TAKEN": "Взята мерчантом",
        "WAITING_PAYMENT": "Ожидает оплату", "WAITING_RECEIPT": "Ожидает чек",
        "WAITING_CHECK": "На проверке", "CONFIRMED": "Подтверждено",
        "REJECTED": "Отклонено", "EXPIRED": "Истекло время",
    }
    return {
        "id": app["id"], "bank_name": bank["bank_name"] if bank else "Unknown",
        "amount_uah": app["amount_uah"], "payment_code": app["payment_code"],
        "status": app["status"], "status_label": STATUS_LABELS.get(app["status"], app["status"]),
        "created_at": app["created_at"], "requisites_sent_at": app["requisites_sent_at"],
        "expires_at": app["expires_at"], "requisites": app["requisites_text_override"]
    }
